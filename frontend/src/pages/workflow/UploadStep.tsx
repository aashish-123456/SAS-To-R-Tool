...
function classifyComplexity(code: string): { complexity: ComplexityInfo; constructs: string[]; procCategories: string[]; clinicalIndicators: string[]; sdtmAdamIndicators: string[] } {
  ...
  if (score >= 30) {
    ...
    badge = 'bg-blue-100 text-[#1a3050] border-blue-200';
  } else {
    level = 1; label = 'Basic';
...
const SeverityIcon: React.FC<{ severity: ValidationIssue['severity'] }> = ({ severity }) => {
  ...
  return <Info className="w-4 h-4 text-[#1f4368] flex-shrink-0" />;
};
      ...
      <div className="space-y-5 max-w-4xl">
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-6">
          <div className="flex items-start gap-4">
            <div className="w-10 h-10 bg-[#eef3f8] rounded-xl flex items-center justify-center flex-shrink-0">
              <Upload className="w-5 h-5 text-[#1f4368]" />
            </div>
            <div>
            ...
            {phase === 'idle' && (
              <div
                ...
                className={`border-2 border-dashed rounded-xl p-10 text-center cursor-pointer transition-all ${
                  isDragActive ? 'border-[#1f4368] bg-[#eef3f8]' : 'border-slate-200 hover:border-[#8aaec9] hover:bg-slate-50'
                }`}
              >
            ...
            {(phase === 'done' || phase === 'blocked') && sasFile && report && (
              <div className="space-y-4">
                ...
                <div className="flex items-center gap-3 p-4 bg-slate-50 rounded-xl border border-slate-200">
                  <div className="w-9 h-9 bg-[#eef3f8] rounded-lg flex items-center justify-center flex-shrink-0">
                    <File className="w-5 h-5 text-[#1f4368]" />
                  </div>
                  <div className="flex-1 min-w-0">
                  ...
                  <div className="divide-y divide-slate-100">
                    ...
                    {report.autoFixes.length > 0 && (
                      <div>
                        ...
                        >
                          <Wrench className="w-4 h-4 text-[#1f4368] flex-shrink-0" />
                          <span className="text-sm font-medium text-slate-700 flex-1">AutoFix Applied</span>
                          <span className="text-xs font-semibold text-[#1f4368]">{report.autoFixes.length} fix(es)</span>
                        ...
                        {showAutoFixDetail && (
                          <div className="px-4 pb-3 space-y-1.5">
                            {report.autoFixes.map(fix => (
                              <div key={fix.code} className="flex items-start gap-2 text-xs text-slate-600 bg-[#eef3f8] rounded-lg p-2.5">
                                <Zap className="w-3 h-3 text-[#1f4368] mt-0.5 flex-shrink-0" />
                        ...
                        {showWarningDetail && (
                          <div className="px-4 pb-3 space-y-2">
                            {report.warnings.map((w, i) => (
                              <div key={i} className={`rounded-lg p-3 text-xs space-y-1 ${w.severity === 'warning' ? 'bg-amber-50 border border-amber-100' : 'bg-[#eef3f8] border border-blue-100'}`}>
                  ...
                  <div className="flex items-start gap-4">
                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 ${
                      ...
                      {report.overallStatus === 'manual_review_required' ? (
                        ...
                        <Wrench className="w-5 h-5 text-[#1f4368]" />
                      ) : (
                    ...
                    <div className="flex-1">
                      <p className={`text-sm font-extrabold tracking-tight ${
                        ...
                        report.overallStatus === 'auto_fixed' ? 'text-[#1a3050]' : 'text-emerald-700'
                      }`}>
                        {report.overallStatus === 'manual_review_required'
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
          <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
            ...
            <button
              ...
              className={`w-12 h-6 rounded-full p-0.5 transition-colors flex-shrink-0 ${datasetEnabled ? 'bg-[#1f4368]' : 'bg-slate-200'}`}
            >
          ...
          {datasetEnabled && (
            <div className="p-6 space-y-3">
              <div {...getDsRootProps()} className="border-2 border-dashed border-slate-200 hover:border-[#8aaec9] rounded-xl p-6 text-center cursor-pointer transition-colors">
        ...
        <button
          ...
          className="w-full flex items-center justify-center gap-2 px-5 py-3.5 bg-[#1f4368] text-white text-sm font-bold rounded-xl hover:bg-[#1a3654] transition-colors disabled:opacity-40 disabled:cursor-not-allowed shadow-sm"
        >
